...
@section('content')
    <div class="content">
        <div class="mt-4">
            ...
            <div class="card">
                <h5 class="card-header"><span data-feather="home" class="me-2"></span>Discover Wayanad</h5>
                <div class="row g-0">
                    <div class="col-md-12">
                        <div class="card-body">
                            ...
                            <h4 class="mt-4 mb-3 text-800">Discover Wayanad</h4>
                            <button class="btn btn-primary float-end mb-2" type="button" data-bs-toggle="modal"
                            ...
                            <table class="table table-bordered border-primary">
                                ...
                                <tbody>
                                    @foreach ($discovers as $discover)
                                        <tr id="detail-row-{{ $discover->id }}">
                                            <td class="p-2">{{ $discover->name }}</td>
                                            <td class="p-2">
                                                <div class="avatar avatar-4xl ">
                                                    @if ($discover->image_url)
                                                        <div class="avatar avatar-4xl ">
                                                            <img class="rounded-none " alt=""
                                                                src="{{ asset('storage/' . $discover->image_url) }}" />
                                                        </div>
                                                    @endif
                                                </div>
                                            </td>
                                            <td class="p-2">{!! $discover->description !!}</td>
                                            <td class="p-2">
                                                <button class="btn btn-phoenix-danger me-1 mb-1 deleteDetails"
                                                    data-id="{{ $discover->id }}" type="button"> Delete
                                                </button>
                                                <button class="btn btn-phoenix-info me-1 mb-1 edit_details" type="button"
                                                    data-bs-target="#verticallyCentered-2" id="{{ $discover->id }}"
        ...
        <div class="modal fade" id="verticallyCentered" tabindex="-1" aria-labelledby="verticallyCenteredModalLabel"
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    ...
                    <form action={{ route('discover-slider-upload') }} method="post" enctype="multipart/form-data" name="Slider_form">
                        @csrf
        ...
        <div class="modal fade" id="verticallyCentered-1" tabindex="-1"
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="verticallyCenteredModalLabel-1">Discover Wayanad</h5><button
                            class="btn p-1" type="button" data-bs-dismiss="modal" aria-label="Close">
                            <span class="fas fa-times fs--1"></span></button>
                    </div>
                    <form action={{ route('discovers.store') }} method="post" enctype="multipart/form-data" name="main_form_create">
                        @csrf
                        <div class="modal-body">
                            <div class="mb-3">
        ...
        <div class="modal fade" id="verticallyCentered-2" tabindex="-1"
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    ...
                    <form action={{ route('discovers.store') }} method="post" enctype="multipart/form-data" name="main_form_edit">
                        @csrf
...
@section('page-js')
    <script>
        ...
        //     $.ajax({
        //         method: 'POST',
        //         url: '{{ route('discovers.destroy', '') }}/' + id,
        //         data: {
        //             "_token": '{!! csrf_token() !!}',
        //             _method: "DELETE",
        //             id: id
        ...
        //     var token = $("meta[name='csrf-token']").attr("content");
        //     $.ajax({
        //         url: "{{ route('delete-discover-slider-image', '') }}/" + id,
        //         success: function() {
        ...
        $(document).on('click', '.edit_details', function() {
            var id = $(this).attr('id');
            var url = "{{ route('discovers.edit', ':id') }}";
